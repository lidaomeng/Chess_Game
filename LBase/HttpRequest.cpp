...
bool HttpRequest::HttpGet(const std::string& url, std::string& result, bool isHttps)
{
	LLOG_DEBUG("http get url :%s", url.c_str());
	...
	if(res != CURLE_OK)
	{
		LLOG_DEBUG("http request fail:%s,%s", url.c_str(), curl_easy_strerror(res));
		return false;
	}
	else
	{
...
bool HttpRequest::HttpPost(const std::string& url, const std::string& postData, std::string& result, bool isHttps)
{
	LLOG_DEBUG("http post:%s", url.c_str());
	...
	if(res != CURLE_OK)
	{
		LLOG_DEBUG("http post fail:%s,%s", url.c_str(), curl_easy_strerror(res));
		return false;
	}
	else
	{
...
void HttpRequest::DeodeUrlParams(const Lstring& str, std::map<std::string, std::string>& params, bool urldecode)
{
