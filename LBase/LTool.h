...
inline void ReadMapValue(msgpack::object& obj, Lstring key, T& t)
{
	...
	if (obj.type ==  MSGPACK_OBJECT_MAP)
	{
		...
		do
		{
			...
			{
				if(pv.type == MSGPACK_OBJECT_ARRAY)
					pv.convert(t);
				break;
}
			++pkv;
...
inline void ReadMapValue(msgpack::object& obj, Lstring key, Lint& value)
{
	...
	if (obj.type ==  MSGPACK_OBJECT_MAP)
	{
		...
		do
		{
			...
			if (pk.as<Lstring>() == key )
			{
				if(pv.type == MSGPACK_OBJECT_POSITIVE_INTEGER)
					pv.convert(value);
				break;
			}
			++pkv;
	...
	value = msgpack::object();
 	...
 	if (obj.type ==  MSGPACK_OBJECT_MAP)
 	{
 		...
 		do
 		{
 			...
 			if (pk.as<Lstring>() == key )
 			{
 				if(pv.type == MSGPACK_OBJECT_ARRAY || pv.type == MSGPACK_OBJECT_MAP)
					value = pv;
 				break;
 			}
 			++pkv;
		...
		if (index < array.via.array.size)
 		{
			pav = array.via.array.ptr + index;
 			if (pav->type ==  MSGPACK_OBJECT_MAP)
 			{
 				pkv = pav->via.map.ptr;
